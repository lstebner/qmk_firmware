...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [LAYER_BASE_ALT] = LAYOUT(
      ...
      KC_TRNS, LGUI(KC_Z), LGUI(KC_X), LGUI(KC_C),  LGUI(KC_V), LGUI(KC_B),  BL_STEP, KC_TRNS, KC_TRNS, VIM_PREV_TAB, VIM_NEXT_TAB, KC_TRNS, KC_TRNS,
      KC_TRNS, XXXXXXX, KC_TRNS,                   KC_UNDERSCORE,                   XXXXXXX, XXXXXXX, XXXXXXX
      ),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    ...
    case TOGGLE_INSPECTOR:
      if (record->event.pressed) {
        SEND_STRING(SS_DOWN(X_LGUI) SS_DOWN(X_LALT) SS_TAP(X_I) SS_UP(X_LALT) SS_UP(X_LGUI));
      }
      break;
